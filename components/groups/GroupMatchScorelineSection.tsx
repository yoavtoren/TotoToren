    ...
    <section className="space-y-4">
      <div>
        ...
        <div className="flex flex-wrap gap-2 mt-1 text-xs">
          ...
          <span className="glass px-2 py-1 rounded-lg"><strong className="text-amber-300">🥅 סך שערים</strong> <span className="text-white/40">= +2 נק׳</span></span>
                ...
                <div className="border-t border-white/10 divide-y divide-white/5">
                  {matches.map((match) => {
                    ...
                    return (
                      ...
                      {/* ── Mobile layout (3-row card) ───────────── */}
                      <div dir="ltr" className="sm:hidden mx-3 my-2 rounded-xl bg-white/5 border border-white/10 overflow-hidden">
                        {/* Row 1: teams + date */}
                        <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
                          <div className="flex items-center gap-1.5 min-w-0 flex-1">
                            {homeTeam && <span className="text-base shrink-0">{getFlagEmoji(homeTeam.flag_code)}</span>}
                            <span className="text-[11px] font-semibold text-white truncate">{match.home}</span>
                          </div>
                          <span className="text-[9px] text-white/25 font-mono shrink-0 px-2">{dateLabel}</span>
                          <div className="flex items-center gap-1.5 min-w-0 flex-1 justify-end">
                            <span className="text-[11px] font-semibold text-white truncate text-right">{match.away}</span>
                        ...
                        <div className="flex items-center justify-center gap-2 px-3 py-2 border-b border-white/10">
                          {(['1','X','2'] as const).map(o => (
                            <button
                              ...
                              disabled={disabled}
                              className={cn(
                                'flex-1 h-9 rounded-lg border text-sm font-bold transition-all',
                                outcome === o
